...
function ValueAt(props) {
  ...
  return (
    <span>
      {`${units.getValueString(
        obsType,
        utility.getNested([obsType, stats, "value"], statsData),
        unitSystem
      )}`}

      {obsType === "wind_speed" &&
        ` from ${units.getValueString(
          "wind_dir",
          utility.getNested(["wind_speed", stats, "dir"], statsData),
          unitSystem
        )}`}

      {` at ${units.getValueString(
        "timestamp",
        utility.getNested([obsType, stats, "timestamp"], statsData),
        unitSystem,
        timeFormat
      )}`}
    </span>
...
function MinMaxValues(props) {
  ...
  return (
    <React.Fragment>
      <tr>
        <td className="label">Low {units.getLabel(obsType)}</td>
        <td className="data">
          <ValueAt
            obsType={obsType}
            statsData={statsData}
      ...
      <tr>
        <td className="label">High {units.getLabel(obsType)}</td>
        <td className="data">
          <ValueAt
            obsType={obsType}
            statsData={statsData}
...
function MaxWind(props) {
  ...
  return (
    <div>
      <tr>
        <td className="label">Max Wind Speed</td>
        <td className="data">
          <td className="data">
            <ValueAt
              obsType="wind_speed"
...
function StatsTable(props) {
  const { statsData, isFetching, header, timeFormat, componentClass: Component } = props;
  ...
  if (
    unitSystem === null ||
    unitSystem !== utility.getNested(["unit_system", "min", "value"], statsData)
  ) {
